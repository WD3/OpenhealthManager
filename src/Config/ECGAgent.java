...
import ieee_11073.part_20601.asn1.SaFlags;
import ieee_11073.part_20601.asn1.SaSpec;
import ieee_11073.part_20601.asn1.SampleType;
...
public class ECGAgent extends Specialization{
	
	public static final int MDC_PRESS_BLD_NONINV = 18498;
	public static final int MDC_DIM_BEAT_PER_MIN = 2720;
	public static final int MDC_DIM_MMHG = 3872;
	
	public static final int MDC_DIM_MILLI_VOLT  = 4274; /* mV */
	public static final int MDC_ECG_ELEC_POTL = 256;/*Unspecified Lead*/
	public static final int MDC_ECG_ELEC_POTL_I  = 257; /*Lead I*/
	public static final int MDC_ECG_ELEC_POTL_II = 258; /*Lead II*/
	public static final int MDC_ECG_ELEC_POTL_III = 317; /*Lead III*/
	public static final int MDC_ECG_ELEC_POTL_AVR = 318; /*Augmented voltage right (aVR)*/
	public static final int MDC_ECG_ELEC_POTL_AVL = 319; /*Augmented voltage left (aVL)*/
	public static final int MDC_ECG_ELEC_POTL_AVF = 320; /*Augmented voltage foot (aVF)*/
	public static final int MDC_ECG_ELEC_POTL_V1 = 259; /*Lead V1*/
	public static final int MDC_ECG_ELEC_POTL_V2 = 260; /*Lead V2*/
	public static final int MDC_ECG_ELEC_POTL_V3 = 261; /*Lead V3*/
	public static final int MDC_ECG_ELEC_POTL_V4 = 262; /*Lead V4*/
	public static final int MDC_ECG_ELEC_POTL_V5 = 263; /*Lead V5*/
	public static final int MDC_ECG_ELEC_POTL_V6 = 264; /*Lead V6*/
	...
	public ECGAgent(){
		...
		configdata.add(generateRtsaConfig());
		Measures = new ArrayList<Map<HANDLE, Object>>();
		Measures.add(generateWave());
	}
	...
	private DeviceConfig generateTestDeviceConfig()
	{
		...
		dev_conf.setSystemId(AgentConfig.bloodpressure_system_id);
	...
	private ConfigObject generateRtsaConfig(){
		Hashtable<Integer,Attribute> attribs = new Hashtable<Integer, Attribute>();
		Attribute attrib;
		ConfigObject confObj = new ConfigObject();
		HANDLE handle = new HANDLE();
		...
		confObj.setObj_handle(handle);
		OID_Type oid= new OID_Type();
		oid.setValue(new INT_U16(Nomenclature.MDC_MOC_VMO_METRIC_SA_RT));
		confObj.setObj_class(oid);
		try {
			//set Handle
			attrib = new Attribute(Nomenclature.MDC_ATTR_ID_HANDLE, confObj.getObj_handle());
			...
			TYPE t =  new TYPE();
			oid= new OID_Type();
			oid.setValue(new INT_U16(MDC_ECG_ELEC_POTL_I));
			t.setCode(oid);
			t.setPartition(new NomPartition(2));
			attrib = new Attribute(Nomenclature.MDC_ATTR_ID_TYPE, t);
			attribs.put(Nomenclature.MDC_ATTR_ID_TYPE, attrib);
			...
			AttrValMapEntry entry = new AttrValMapEntry();
			oid= new OID_Type();
			oid.setValue(new INT_U16(Nomenclature.MDC_ATTR_SIMP_SA_OBS_VAL));
			entry.setAttribute_id(oid);
			entry.setAttribute_len(10);//measure data 50*8bits
			avm.add(entry);
			attrib = new Attribute(Nomenclature.MDC_ATTR_ATTRIBUTE_VAL_MAP, avm);
			attribs.put(Nomenclature.MDC_ATTR_ATTRIBUTE_VAL_MAP, attrib);
			//set metric_spec_small 
			MetricSpecSmall mss = new MetricSpecSmall();
			mss.setValue(new BitString(new byte[]{(byte)0x00,(byte)0x00}));
			attrib = new Attribute(Nomenclature.MDC_ATTR_METRIC_SPEC_SMALL, mss);
			attribs.put(Nomenclature.MDC_ATTR_METRIC_SPEC_SMALL, attrib);
			//set sample period
			RelativeTime rt = new RelativeTime();
			rt.setValue(new INT_U32(16l));
			attrib = new Attribute(Nomenclature.MDC_ATTR_TIME_PD_SAMP, rt);
			attribs.put(Nomenclature.MDC_ATTR_TIME_PD_SAMP, attrib);
			//set sample specification
			SaSpec ss = new SaSpec();
			ss.setArray_size(10);
			ss.setFlags(new SaFlags(new BitString(new byte[]{0x00,0x00})));
			SampleType st = new SampleType();
			st.setSample_size(new INT_U8(8));
			st.setSignificant_bits(255);
			ss.setSample_type(st);
			attrib = new Attribute(Nomenclature.MDC_ATTR_SA_SPECN, ss);
			attribs.put(Nomenclature.MDC_ATTR_SA_SPECN, attrib);
			//set scale and range specification
			ScaleRangeSpec8 srs8 = new ScaleRangeSpec8();
			FLOAT_Type ft = new FLOAT_Type();
			ft.setValue(new INT_U32(0l));
			//TODO:set ft value
			srs8.setLower_absolute_value(ft);
			srs8.setUpper_absolute_value(ft);
			srs8.setLower_scaled_value(0);
			srs8.setUpper_scaled_value(0);
			attrib = new Attribute(Nomenclature.MDC_ATTR_SCALE_SPECN_I8, srs8);
			attribs.put(Nomenclature.MDC_ATTR_SCALE_SPECN_I8, attrib);
		} catch (InvalidAttributeException e) {
			e.printStackTrace();
		}
		try {
	...
	private ConfigObject generateMDSConfig(){
		...
		try {
			...
			attribs.put(Nomenclature.MDC_ATTR_ID_HANDLE, attrib);
			//set system id
			attrib = new Attribute(Nomenclature.MDC_ATTR_SYS_ID, "ecg sys id".getBytes());
			attribs.put(Nomenclature.MDC_ATTR_SYS_ID, attrib);
			//set Mds-Time_info		
	...
	private Map<HANDLE,Object> generateWave(){
		Map<HANDLE,Object> measure = new Hashtable<HANDLE, Object>();
		HANDLE handle = new HANDLE();
		handle.setValue(new INT_U16(2));
		ArrayList<Object> objlist = new ArrayList<Object>();
