...
public class OximeterAgent extends Specialization{
	...
	private ConfigObject generateNumericConfig1(){
		...
		try {
			...
			oid= new OID_Type();
			oid.setValue(new INT_U16(Nomenclature.MDC_ATTR_NU_VAL_OBS_BASIC));
			entry.setAttribute_id(oid);
			entry.setAttribute_len(2);
			avm.add(entry);		
	...
	private Map<HANDLE,Object> generateMeasure(){
		...
		bnv.setValue(new SFLOAT_Type(80));
